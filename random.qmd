...
df2 %>% 
  ...
  theme_bw() +
  xlab("År") +
  ylab("Tusen kroner per person") +
  ggtitle("BNP i løpende og fastepriser i 2015")
```

*\[Besvar oppgave 1b her\]* Grafen viser utviklingen til BNP fra år 2000 til år 2022. BNP måles i løpende priser, mens MEMOBNP måles i en fast pris fra 2015. BNP stiger mer enn MEMOBNP i denne grafen. Intektene har steget. BNP viser utviklingen til priser bedre, mens MEMOBNP viser utviklingen til produksjonen bedre. BNP brukes til å sammenligne med inntekstsvekst og MEMOBNP brukes til å sammenligne med produksjonsvekst.
...
*\[Besvar oppgave IIa her\]* df_wide omformaterte df2 slik at "tid", "BNP" og "MEMOBNP" ble til kolonner i stedet for "var", "tid" og "verdi". Nå kan BNP og MEMOBNP sammenlignes side om side og ikke over og unner med flere rad mellom ønsket sammenlignings punkt.

df_wide omorganiserte df2 slik at "tid", "BNP" og "MEMOBNP" ble til kolonner i stedet for "var", "tid" og "verdi". I kolonne "tid" er verdiene fra 1970 til 2022 bare en gang i stedet for to. I kolonne "BNP" finner vi bare verdiene til "BNP". I kolonne "MEMOBNP" finner vi bare verdiene til "MEMOBNP".
...
# Besvar oppgave IIb her 
  # Denne koden lager ny kolonne(DBNP) som resulterer med å subtraktere BNP med LBNP.
df_wide <- df_wide %>% 
  mutate("DBNP" = BNP - LBNP) %>% 
  mutate("DMEMOBNP" = MEMOBNP - LMEMOBNP) %>% 
  relocate("DBNP", .before = "MEMOBNP")

  #Denne koden lager ny kolonne(PBNP) som resulterer med å dividere DBNP med BNP og multiplisere med 100.
df_wide <- df_wide %>% 
  mutate("PBNP" = DBNP/BNP*100) %>% 
  mutate("PMEMOBNP" = DMEMOBNP/MEMOBNP*100) %>% 
  relocate("PBNP", .before = "MEMOBNP")
```
...
df_long %>% 
  ...
  xlab("År") +
  ylab("Prosent") +
  ggtitle("Prosent vekst i nominelt og reelt BNP per peron") +
  scale_color_discrete(labels = c("Nominelt BNP", "Reelt BNP"))
```
