...
```{r}
url <- "https://raw.githubusercontent.com/uit-sok-1004-h23/uit-sok-1004-h23.github.io/main/data/oecd_data_nor.csv"
...
# Calculate quartiles for income before tax
df_before_tax <- tromso %>%
  select(bruttonetto) %>%
  quantile(probs = seq(0.25, 1, by = 0.25), na.rm = TRUE) %>%
  as_tibble() %>%
  add_column(Quartile = c(25, 50, 75, 100)) %>%
  rename(inntekt_fr_skatt = value)

# Plot income before tax
df_before_tax %>%
  ggplot(aes(x = Quartile, y = inntekt_fr_skatt / 1000)) +
  geom_col() +
  labs(title = "Inntektsfordeling i Tromsø 2015 (før skatt)",
       y = "Inntekt (1000kr)", x = "Quartile") +
  theme_minimal()

# Plot income after tax
df_after_tax %>%
  ggplot(aes(x = Quartile, y = inntekt_etter_skatt / 1000)) +
  geom_col() +
  labs(title = "Inntektsfordeling i Tromsø 2015 (etter skatt)",
       y = "Inntekt (1000kr)", x = "Quartile") +
  theme_minimal()
